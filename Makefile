...
build/bench_hamming_control: tools/bench_hamming_control.c build/qdalign.o include/qdalign.h | build
	$(CC) $(CFLAGS) tools/bench_hamming_control.c build/qdalign.o -o $@ $(LDFLAGS)

bench-10x-control: build/bench_hamming_control
	./build/bench_hamming_control $${DOTMATCH_10X_READS:-1000000} $${DOTMATCH_10X_TARGETS:-4096} $${DOTMATCH_10X_LENGTH:-20} $${DOTMATCH_10X_K:-1} $${DOTMATCH_10X_REPEATS:-5}
...
bench-barcode-panel-design:
	python3 scripts/bench_barcode_panel_design.py

barcode-panel-design-gate:
	python3 scripts/check_barcode_panel_design_gate.py
...
bench-crispr-hamming-k23: dotmatch
	python3 scripts/run_crispr_hamming_k23_comparison.py --manifest examples/crispr_sanson_brunello/data/manifest.json --requested-records-per-sample "$${DOTMATCH_K23_RECORDS_PER_SAMPLE:-100000}"
...
workflow-integration-test: dotmatch shared
	python3 scripts/run_workflow_integration_tests.py
...
download-metrics:
	python3 scripts/check_download_metrics.py --output docs/download-metrics.json

joss-paper-ready:
	python3 scripts/check_joss_paper.py

repro repro-small:
	python3 scripts/repro_small.py
