...
`dotmatch_crispr_count.xml` keeps the DotMatch command interface, and
...
The wrapper exposes a two-sample CRISPR guide-counting interface: guide library,
...
`crispr_qc.html`, `crispr_qc.json`, `crispr_qc.summary.tsv`, counts, and
summary JSON. The assay report is the main report for review;
