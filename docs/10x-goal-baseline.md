...
| Scientific impact | `docs/workflow-adoption.json` records one released Snakemake wrapper on `v9.17.1`. `docs/adopters/` still contains only the record template. Open external PRs are not counted as adoption. | One released workflow-manager integration and zero independently authored lab use records. The 10x operational target remains ten independently authored, public-safe scientific evaluation or use records. |
...
3. Land the open nf-core, MultiQC, and Galaxy submissions, then convert each
   accepted/released integration into `docs/workflow-adoption.json`.
4. Invite independent scientific and industry evaluators through
   ...
   from replies alone.
5. Prefer Bioconda PR #68663 over the autobump #68629 so DotMatch 0.4.0 and the
   AssayCode metapackage land together after osx-64 CI is green.
