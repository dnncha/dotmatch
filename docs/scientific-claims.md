...
| Package-channel availability. | `make python-package-test` verifies local Linux/macOS wheel and sdist installability; the v0.1.0 Bioconda recipe PR has passed upstream CI. | PyPI, the merged Bioconda package, Docker registry, BioContainers, and Zenodo release artifacts are separate distribution channels and should not be claimed until `make distribution-channels` passes. |
