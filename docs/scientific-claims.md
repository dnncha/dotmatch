...
| --- | --- | --- |
| Barcode demultiplexing state of the art. | `make barcode-sota-gate` currently fails. | The checked artifact lacks claim-grade real repeated rows with assigned reads, Cutadapt repeated rows, and a second comparator such as Ultraplex/Je or an exact hash splitter for `k=0`. |
