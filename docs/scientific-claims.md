...
- an experimental Phred posterior helper in the Python API for deterministic,
  single-window assignment with explicit ambiguous posterior-mass reporting;
...
- AssayScript v2 compilation and execution are experimental: the compiler
  validates multi-read declarations, fingerprints inputs, selects deterministic
  strategies, and writes portable JSON plans. The Python runtime synchronizes
  FASTQs, extracts fixed or anchored windows, preserves ambiguity, filters
  allowed tuples, performs deterministic joint rescue, and commits auditable
  outputs atomically. It has unit evidence, not yet public multi-read dataset or
  throughput evidence, and is not a production universal runtime;
- the `dotmatch.calibration` module is experimental: it fits per-cycle error
  summaries from independently trusted pairs, combines empirical and Phred
  evidence, supports selective and allowed-tuple decoding, and reports
  calibration metrics. It has not yet passed public-data calibration or
  throughput gates and does not replace the deterministic CLI path;
- `assaycode simulate` is an experimental substitution-only design aid. It
  deterministically estimates panel yield, ambiguity, no-calls, false-discovery
  rate, and confusion under declared error rates; it is not empirical sequencing
  performance evidence or a platform model;
...
| The Python API exposes an experimental quality-aware posterior helper for one fixed-window read against known targets. | Experimental | `python/tests/test_matcher.py` posterior-assignment cases | This is a simple Phred likelihood helper with literal-byte target comparison and optional priors. It is not yet calibrated on public datasets, not used by the high-throughput CLI, and not a speed claim. |
...
| DotMatch has checked public multi-guide direct-capture Perturb-seq per-read assignment evidence. | Supported | `make perturb-seq-case-study-fixture-gate`, `make perturb-seq-case-study-public-gate`, report at `docs/benchmarks/perturb_seq_gse146194/README.md`; the earlier single-guide extraction lane remains at `docs/benchmarks/perturb_seq/README.md` | Applies to the frozen 48,000-read held-out SRR11214031 prefix and the published 32-guide GBC list under exact and Hamming `k=1` fixed-window rules. The gate requires zero per-read status, target, and distance differences from matched independent oracles. This is not Cell Ranger parity, cell-barcode/UMI processing, guide-per-cell calling, expression analysis, perturbation-effect inference, full-run prevalence, or speed-comparison evidence. |
...
| Perturb-seq comparisons. | `make perturb-seq-case-study-public-gate` pins the pre-analysis protocol, accessions, derived-input hashes, 32-guide library, 48,000 held-out reads, k=0/k=1 outcome counts, and zero per-read differences from matched exact-slice and exhaustive Hamming-radius oracles. `make perturb-seq-case-study-fixture-gate` separately requires exact, corrected, ambiguous, unmatched, and invalid outcomes. | The supported claim is per-read fixed-window assignment on the bounded GSE146194/SRR11214031 case study. The public slice has zero ambiguous reads and the library has minimum pairwise Hamming distance 8; ambiguity behavior is therefore demonstrated by the deterministic fixture, not inferred from the public slice. Guide-per-cell, cell-barcode/UMI, expression, perturbation-effect, full-run, biological-validity, and speed claims remain outside this evidence. |
