...
| Package-channel availability. | `make python-package-test` verifies local Linux/macOS wheel and sdist installability; Bioconda has accepted DotMatch 0.1.4 as the latest public package before the 0.1.5 release. | Package-channel availability is distribution evidence only. It does not extend CRISPR, barcode, feature-barcode, BCL, amplicon, adapter, GPU, or aligner-comparison claims; `make distribution-channels` records each release channel only after public metadata and install smoke tests pass. |
