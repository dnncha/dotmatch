...
def test_assay_handoff_writes_review_bundle_with_input_and_output_hashes(tmp_path: Path) -> None:
    ...
    assert {item["role"] for item in record["inputs"]} == {"targets", "sample:sample_a", "sample:sample_b"}
    ...
    assert "It does not include raw reads" in (bundle / "README_FOR_REVIEW.md").read_text(encoding="utf-8")
