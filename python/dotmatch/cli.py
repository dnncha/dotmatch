...
def command_crispr_quickstart(args: argparse.Namespace) -> int:
    ...
    invalid = [
        ...
        if not any(path.name.lower().endswith(suffix) for suffix in FASTQ_SUFFIXES)
    ]
    if invalid:
    ...
    if os.path.lexists(staging_parent):
        print(
            f"dotmatch crispr: refusing to overwrite existing staging directory: {staging_parent}",
            file=sys.stderr,
        )
        return 2
