...
def _write_handoff_bundle(plan: AssayPlan, out_dir: Path) -> None:
    ...
    if not manifest_path.is_file() or not reliability_path.is_file():
        raise AssaySpecError(
            "handoff requires a completed assay run with assay_manifest.json and reliability_summary.json; "
            "run 'dotmatch assay start <spec>' first"
        )
    ...
    for key in (
        "normalized_spec", "manifest", "manifest_summary", "reliability_summary",
        "reliability_findings", "reliability_report", "reliability_manifest_summary",
        "assay_fixes", "methods", "citation_bib", "software_versions", "assay_report",
        "counts", "sample_qc", "summary", "report", "crispr_qc", "crispr_qc_summary",
        "crispr_qc_report", "target_counts_long",
    ):
        ...
        copied.append({"role": key, "path": str(destination.relative_to(out_dir)), "sha256": _sha256_file(destination), "bytes": destination.stat().st_size})

    input_files = [
        {"role": role, "declared_path": declared, "sha256": _sha256_file(path), "bytes": path.stat().st_size}
    ...
    bundle_manifest = {
        ...
        "inputs": input_files,
        "review_files": copied,
        "review_boundary": "Raw reads are not copied into this bundle. Verify the declared input hashes in the originating controlled workspace.",
    }
    (out_dir / "handoff_manifest.json").write_text(json.dumps(bundle_manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    _write_handoff_readme(out_dir, bundle_manifest)
    checksum_rows = [
        f"{entry['sha256']}  {entry['path']}" for entry in copied
    ]
    (out_dir / "SHA256SUMS").write_text("\n".join(checksum_rows) + ("\n" if checksum_rows else ""), encoding="utf-8")


def _handoff_inputs(assay: AssaySpec) -> list[tuple[str, str, Path]]:
    if assay.mode == "count":
        items = [("targets", str(assay.data["targets"]))]
        items.extend((f"sample:{sample['id']}", str(sample["fastq"])) for sample in _samples(assay.data))
    elif assay.mode == "demux":
        items = [("barcodes", str(assay.data["barcodes"])), ("reads", str(assay.data["reads"]))]
    else:
        items = [
    ...
    return [(role, declared, _path_from_spec(assay.path, declared, allow_absolute=True, name=role)) for role, declared in items]
