#!/usr/bin/env python3
"""Generate native Edlib comparison graphs for README headline benchmarks."""
...
def write_report(df: pd.DataFrame, speedups: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Claim Discipline",
        "",
        "These are native Edlib scan comparisons for exact short-DNA assignment workloads, plus simple exact-hash and BK-tree baselines. Do not lead with `k=0` speedups against Edlib scan; exact lookup must be judged against hash-table baselines. The publishable wedge is `k=1` indexed assignment with zero correctness disagreements.",
        "",
    ]
