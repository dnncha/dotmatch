#!/usr/bin/env python3
"""Generate CRISPR SOTA evidence report from gate-grade raw CSVs."""
...
from check_crispr_sota_gate import FULL_FASTQ_SAMPLE_READS


ROOT = Path(__file__).resolve().parents[1]
RAW = ROOT / "benchmarks" / "raw"
OUT_DIR = ROOT / "docs" / "benchmarks" / "crispr_sota"
...
def svg_bars(stats: list[dict[str, str]], path: Path) -> None:
    ...
    parts = [
        ...
        '<text class="title" x="20" y="28">CRISPR SOTA repeated real-data throughput</text>',
...
def main() -> None:
    repeated = read_rows(RAW / "crispr_sota_repeated.csv")
    validation = read_rows(RAW / "crispr_sota_edlib_validation.csv")
    agreement = read_rows(RAW / "crispr_sota_count_agreement_summary.csv")
    stats = repeated_stats(repeated)
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    svg_bars(stats, FIG_DIR / "crispr_sota_throughput.svg")

    full_rows = [r for r in stats if r["records_per_sample"] == "full"]
    subsample_rows = [r for r in stats if r["records_per_sample"] != "full"]
    content = [
        "# CRISPR SOTA Evidence",
        "",
        "This report is generated from raw CSV artifacts. It is intentionally stricter than the public smoke report: SOTA claims require both MAGeCK/Yusa and Sanson/Brunello real-data rows, competitor rows, count agreement, and Edlib validation.",
        "",
        "## Claim Discipline",
        "",
        ...
        "- Broad SOTA wording is blocked unless `make crispr-sota-gate` passes.",
        "",
        "## Throughput Figure",
        "",
        "![CRISPR SOTA throughput](" +
        markdown_link_path(FIG_DIR / "crispr_sota_throughput.svg", OUT_DIR) + ")",
        "",
        ...
        "## Raw Inputs",
        "",
        "- `benchmarks/raw/crispr_sota_repeated.csv`",
        "- `benchmarks/raw/crispr_sota_edlib_validation.csv`",
        "- `benchmarks/raw/crispr_sota_count_agreement_summary.csv`",
    ]
