...
def run_public_exact_baseline(metadata_path: Path, metadata: dict, targets: Path, reads: Path) -> dict[str, str]:
    ...
    return {
        ...
        "validation_notes": "simple exact substring comparison over the fixed public adapter-prefix window",
        "command": command_text(cmd),
        "counts": "",
        "summary": "",
        "assignments": "",
...
def write_report(rows: list[dict[str, str]], report: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "## Scope",
            "",
            "These lanes verify fixed-window known-oligo/adapter assignment, one-substitution rescue, and explicit ambiguous/unmatched diagnostics. Run `make oligo-adapter-smoke-gate` for the small fixture and `make oligo-adapter-public-gate` for the public lane. The public lane checks adapter-prefix assignment for the recorded R1 window. Primer removal, UMI grouping, read merging, and adapter-trimming workflows need separate comparison records.",
            "",
        ]
    )
